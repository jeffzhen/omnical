...
for p, pol in zip(range(len(wantpols)), wantpols.keys()):
	print FILENAME + " MSG:",  data[:len(t), p].shape
	if needrawcal:
		(np.transpose(data[:len(t), p],(0,2,1))/rawcorrection[p, np.newaxis,:,:]).tofile('miriadextract_' + pol + '_' + ano)
	else:
		np.transpose(data[:len(t), p],(0,2,1)).tofile('miriadextract_' + pol + '_' + ano)	
